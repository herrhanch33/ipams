...
class IPTable(db.Model):
    ...
    IPAddress = db.Column('IP Address', db.String(255))
...
# Function to connect to the Access database using pyodbc
def connect_to_access_database():
	try:
...
def ip_management():
    ...
    if request.method == 'POST':
        ...
        if query:
            query_no_periods = query.replace('.', '')
            ...
            results = IPTable.query.filter(
                or_(
                    ...
                    concat(IPTable.Class_, IPTable.Gateway, IPTable.IPAddress).ilike(f'%{query_no_periods}%'),
                    concat(IPTable.Class_, IPTable.Gateway, IPTable.IPAddress).ilike(f'%{query}%')
                )
...
def add():
    ...
    per_page = 510  # Adjust as per your pagination needs
